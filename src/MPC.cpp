...
std::vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  vector<double> rslt;
  rslt.push_back(solution.x[delta_start]);
  rslt.push_back(solution.x[a_start]);

  //Add the N-1 results of x and y from the solution as well
  for (int i = 0; i < N-1; i++) {
    rslt.push_back(solution.x[x_start + i + 1]);
    rslt.push_back(solution.x[y_start + i + 1]);
  }
